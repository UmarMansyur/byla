...
<div class="app-section st1 mt-1 mb-5 bg_white_color">
  <div class="tf-container">
    ...
    <div class="text-end">
      ...
      <a href="/history/export?startDate={{ request('startDate') }}&endDate={{ request('endDate') }}" class="btn btn-success" style="width: 50px; height: auto;">
        <i class="bx bx-download"></i>
      </a>
    </div>
    <h3 class="my-3">Daftar Transaksi</h3>
    <div class="trading-month infinite-scroll mt-5 mb-3">
      @foreach ($transactions as $transaction)
      <div class="group-trading-history mb-2 pb-2 border-bottom">
        <a class="tf-trading-history" href="{{ route('History Detail Page', Crypt::encryptString($transaction->id)) }}">
          <div class="inner-left">
            <div class="icon-box">
              <i class="bx bx-store text-primary" style="font-size: 1.5rem"></i>
            </div>
            <div class="content">
              <h4>{{ $transaction->merchant->name }}</h4>
              <h6 class="text-muted">{{ $transaction->merchant->merchant_code }}</h6>
              <p>{{ $transaction->updated_at->format('d M Y H:i') }} WIB</p>
            </div>
          </div>
          @if ($transaction->status == 'success')
          <span class="num-val {{ $transaction->user_id == Auth::user()->id ? 'text-success' : 'text-danger' }}"
          ...
          <span class="num-val warning_color" style="font-size: 12px;">
            Rp. {{ number_format($transaction->nominal, 0, ',', '.') }}
          </span>
          @elseif ($transaction->status == 'failed')
          <span class="num-val critical_color" style="font-size: 12px;">
            - Rp. {{ number_format($transaction->nominal, 0, ',', '.') }}
          </span>
          @endif
        </a>
